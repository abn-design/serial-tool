...
- 串口设备列表下拉选择，支持刷新
- 波特率下拉选择（8 档预设），支持直接输入自定义波特率
- 数据位（5/6/7/8）、停止位（1/2）、校验位（无/偶/奇）配置
- DTR / RTS 控制线开关（打开串口后可用）
- 打开 / 关闭串口，打开后禁用端口相关配置
- 打开失败时显示明确错误原因
- 上方滚动显示接收数据（HEX + 文本）
- 下方支持多条发送输入（多行文本框），默认 1 条，最后一条后提供 `Add`
- 发送可选末尾追加换行（无 / CRLF / LF）
- `Send` 支持单次发送
- 勾选“持续发送”后显示发送周期下拉，首次点击 `Send` 开始持续发送，按钮切换为 `Stop`
- 接收/发送日志可一键清空，发送输入可一键清空
- 设备、波特率、串口参数、HEX 显示、发送间隔、换行方式等配置自动持久化到配置文件
